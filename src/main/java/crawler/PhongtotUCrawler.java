...
public class PhongtotUCrawler {
    public List<Sample> getSampleHostelDataFromPhongTot() {
        ...
        try {
            ...
            Integer pageNum = Integer.parseInt(phongtotDocForPage.select(jacksonObj.readYamlForPhongtot().getPageNum()).text());
            ...
            for (int pageNumber = 0; pageNumber < pageNum; pageNumber++) {
                ...
                Document phongtotDoc = Jsoup.connect(PHONG_TOT + "&page=" + pageNumber).timeout(50000).userAgent("Mozilla/5.0 " +
                        "(Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.1916.153 Safari/537.36").get();
                Elements urlList = phongtotDoc.select(jacksonObj.readYamlForPhongtot().getUrlList());
                for (Element sampleElement : urlList) {
                    ...
                    if (postAt >= lastPostAt) {
                        ...
                        //street
                        String streetName = sampleElement.select(jacksonObj.readYamlForPhongtot().getStreetName()).text();
...
//                    System.out.println("Post At : "+ postAt);
                        ...
                        Elements detailUrlList = detailDoc.select(jacksonObj.readYamlForPhongtot().getDetailUrlList());
